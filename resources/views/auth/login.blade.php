{{-- EXISTING CODE UNCHANGED: extends layouts.app, all form fields, CSRF, validation, and submit logic preserved --}}
{{-- MODIFY ONLY: Visual styling to match dark navy SwiftShip theme --}}
...
<head>
  ...
  <title>Admin Login — {{ config('app.name', 'SwiftShip') }}</title>
...
<body class="min-h-screen flex items-center justify-center px-4 py-12" style="background: linear-gradient(135deg, #0f172a 0%, #0c1a3a 50%, #0f172a 100%); font-family: Inter, ui-sans-serif, system-ui, sans-serif;">
  ...
  <div class="relative z-10 w-full max-w-md">
    ...
    <div class="rounded-2xl border border-slate-800 bg-slate-900/80 p-8 shadow-2xl shadow-black/50 backdrop-blur-sm">
      ...
      {{-- EXISTING CODE UNCHANGED: form action, method, CSRF, all fields preserved --}}
      <form method="POST" action="{{ route('login') }}" class="space-y-5">
        ...
        <label class="block space-y-1.5">
          ...
          <input
            name="email"
            type="email"
            class="w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-slate-100 outline-none transition focus:border-amber-400 focus:ring-2 focus:ring-amber-400/20 @error('email') border-red-500/60 @enderror"
        ...
        {{-- EXISTING CODE UNCHANGED: error display logic --}}
        @if($errors->any())
    ...
    {{-- ADD THIS: Back to Homepage link --}}
