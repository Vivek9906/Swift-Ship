...
<nav class="space-y-1">
    @foreach($items as $item)
        ...
        <a href="{{ route($item['route']) }}" class="group flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold transition-all duration-200
                      {{ $active
            ? 'bg-amber-400/15 text-amber-400 border border-amber-400/20'
            : 'text-slate-400 hover:bg-slate-800 hover:text-white border border-transparent' }}">
            <svg class="h-5 w-5 shrink-0" viewBox="0 0 24 24" fill="{{ str_contains($item['icon'], 'M12 4v16') ? 'none' : 'currentColor' }}"
                 stroke="{{ str_contains($item['icon'], 'M12 4v16') || str_contains($item['icon'], 'M21 21') || str_contains($item['icon'], 'M12 14') ? 'currentColor' : 'none' }}"
