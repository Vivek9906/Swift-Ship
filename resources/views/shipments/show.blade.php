    ...
    <section class="space-y-6">
        <div class="ops-panel rounded-lg p-5">
            <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div>
                    ...
                    <h2 class="mt-1 text-2xl font-bold">{{ $shipment->sender_city }} → {{ $shipment->receiver_city }}</h2>
                    <p class="mt-2 text-sm text-slate-400">{{ $shipment->sender_name }} to {{ $shipment->receiver_name }}</p>
                </div>
            ...
            <dl class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div><dt class="text-xs uppercase text-slate-500">Carrier</dt><dd class="mt-1 font-semibold">{{ $shipment->carrier->name }}</dd></div>
                <div><dt class="text-xs uppercase text-slate-500">Weight</dt><dd class="mt-1 font-semibold">{{ $shipment->weight }} kg</dd></div>
                <div><dt class="text-xs uppercase text-slate-500">Dimensions</dt><dd class="mt-1 font-semibold">{{ $shipment->dimensions }}</dd></div>
                <div><dt class="text-xs uppercase text-slate-500">Cost</dt><dd class="mt-1 font-semibold">₹{{ number_format($shipment->cost, 2) }}</dd></div>
                <div><dt class="text-xs uppercase text-slate-500">ETA</dt><dd class="mt-1 font-semibold">{{ $shipment->estimated_delivery?->format('M d, Y H:i') }}</dd></div>
                <div><dt class="text-xs uppercase text-slate-500">Coordinates</dt><dd class="mt-1 font-mono text-sm">{{ $shipment->current_lat }}, {{ $shipment->current_lng }}</dd></div>
                <div class="sm:col-span-2"><dt class="text-xs uppercase text-slate-500">Receiver Address</dt><dd class="mt-1 font-semibold">{{ $shipment->receiver_address }}</dd></div>
            </dl>
        </div>
        ...
        <div class="ops-panel rounded-lg p-5">
            <div class="mb-4 flex items-center justify-between">
                ...
                <span class="font-mono text-xs text-amber-200">{{ $shipment->status === 'out_for_delivery' || $shipment->status === 'arrived_at_city' ? 'CITY_ZOOM' : 'INDIA_ROUTE' }}</span>
            </div>
    ...
    <aside class="space-y-6">
        <div class="ops-panel rounded-lg p-5">
            ...
            <div class="mt-4 space-y-2 text-sm">
                <p class="text-lg font-bold">{{ $shipment->customer->name }}</p>
                <p class="text-slate-400">{{ $shipment->customer->email }}</p>
                <p class="text-slate-400">{{ $shipment->customer->phone }}</p>
                <p class="text-slate-400">{{ $shipment->customer->city }}</p>
            </div>
            <a href="{{ route('customers.show', $shipment->customer) }}" class="mt-4 inline-flex text-sm font-semibold text-blue-300 hover:text-blue-200">Open profile</a>
        </div>

        <div class="ops-panel rounded-lg p-5">
            <h2 class="mb-4 font-semibold">Order History</h2>
            <div class="space-y-3">
                @foreach($shipment->customer->shipments->take(6) as $order)
                    <a href="{{ route('shipments.show', $order) }}" class="block rounded border border-slate-800 p-3 hover:bg-slate-800/50">
