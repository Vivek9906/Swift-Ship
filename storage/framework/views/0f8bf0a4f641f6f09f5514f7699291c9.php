...
<div class="space-y-5" x-data="{ addOpen: false, edit: null }">
    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <form method="GET" class="grid gap-3 sm:grid-cols-3">
            <input name="search" value="<?php echo e(request('search')); ?>" placeholder="Search customer" class="ops-input">
        ...
        <?php if(auth()->user()?->canManage()): ?>
            <button class="ops-button" @click="addOpen = true">Add Customer</button>
        <?php endif; ?>
    </div>
                ...
                <tbody>
                    <?php $__currentLoopData = $customers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $customer): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr class="border-b border-slate-800 hover:bg-slate-800/60">
                            <td class="px-4 py-3 font-semibold"><a href="<?php echo e(route('customers.show', $customer)); ?>" class="hover:text-blue-200"><?php echo e($customer->name); ?></a></td>
                            <td class="px-4 py-3"><?php echo e($customer->email); ?></td>
                            <td class="px-4 py-3"><?php echo e($customer->phone); ?></td>
                            <td class="px-4 py-3"><?php echo e($customer->city); ?></td>
                            <td class="px-4 py-3"><?php echo e($customer->shipments_count); ?></td>
                            <td class="px-4 py-3"><?php echo e(optional($customer->shipments()->latest()->first())->created_at?->format('M d, Y') ?? 'No orders'); ?></td>
...
<?php endif; ?></td>
                            <td class="px-4 py-3 text-right">
                                <?php if(auth()->user()?->canManage()): ?>
                                    <button class="text-sm font-semibold text-blue-300" @click='edit = <?php echo json_encode($customer, 15, 512) ?>'>Edit</button>
                                <?php endif; ?>
                            </td>
                        </tr>
    ...
    <div x-show="addOpen || edit" x-transition class="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4" style="display:none">
        <form method="POST" :action="edit ? `/customers/${edit.id}` : '<?php echo e(route('customers.store')); ?>'" class="ops-panel w-full max-w-2xl space-y-4 rounded-lg p-5" @click.outside="addOpen = false; edit = null">
            <?php echo csrf_field(); ?>
            <template x-if="edit"><input type="hidden" name="_method" value="PUT"></template>
            <div class="flex items-center justify-between">
                <h2 class="text-lg font-bold" x-text="edit ? 'Edit Customer' : 'Add Customer'"></h2>
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <textarea name="address" class="ops-input md:col-span-2" rows="3" placeholder="Address" x-text="edit?.address" required></textarea>
                <textarea name="notes" class="ops-input md:col-span-2" rows="3" placeholder="Notes" x-text="edit?.notes"></textarea>
            </div>
            <div class="flex justify-end"><button class="ops-button">Save Customer</button></div>
        </form>
    </div>
</div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\bhard\Documents\Codex\2026-05-13\build-me-a-full-logistics-delivery\resources\views/customers/index.blade.php ENDPATH**/ ?>