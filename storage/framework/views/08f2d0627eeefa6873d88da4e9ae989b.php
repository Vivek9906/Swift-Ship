...
<?php $__env->startSection('page_title', $shipment ? 'Track ' . $shipment->tracking_number : 'Shipment Not Found'); ?>
<?php $__env->startSection('meta_description', 'Track your shipment in real-time with SwiftShip.'); ?>
      ...
      <?php if(!$shipment): ?>
        <div class="mx-auto max-w-lg text-center py-20">
          ...
          <p class="font-mono text-lg text-red-400 mb-8 bg-red-500/10 rounded-lg px-4 py-2 inline-block">
            <?php echo e($trackingNumber); ?></p>
          <form method="POST" action="<?php echo e(route('tracking.lookup.submit')); ?>"
            class="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            <?php echo csrf_field(); ?>
            <input name="tracking_number" value="<?php echo e($trackingNumber); ?>"
          ...
          <div class="rounded-2xl border border-slate-800 bg-slate-900/80 p-6">
            <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div>
                ...
                <p class="font-mono text-2xl font-bold text-amber-400 tracking-wider mb-3"><?php echo e($shipment->tracking_number); ?>

                </p>
                <div class="flex items-center gap-3 flex-wrap">
                  <span class="text-xl font-bold text-white"><?php echo e($shipment->sender_city); ?></span>
                  ...
                  <span class="text-xl font-bold text-white"><?php echo e($shipment->receiver_city); ?></span>
                </div>
                <p class="mt-2 text-xs text-slate-500">
                  For <span class="text-slate-300 font-medium"><?php echo e($shipment->receiver_name); ?></span>
                  · ETA <span
                    class="text-slate-300 font-medium"><?php echo e($shipment->estimated_delivery?->format('M d, Y H:i')); ?></span>
                </p>
              </div>
              <?php if (isset($component)) { $__componentOriginal8c81617a70e11bcf247c4db924ab1b62 = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginal8c81617a70e11bcf247c4db924ab1b62 = $attributes; } ?>
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.status-badge','data' => ['status' => $shipment->status,'class' => 'text-sm px-3 py-1.5']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['status' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($shipment->status),'class' => 'text-sm px-3 py-1.5']); ?>
...
<?php endif; ?>
              ...
              <dl class="space-y-4">
                <div class="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <dd class="text-sm font-semibold text-white"><?php echo e($shipment->carrier?->name ?? 'Assigning carrier'); ?></dd>
                  </div>
                </div>
                <div class="flex items-start gap-3">
                  <span class="text-base mt-0.5">📍</span>
                  <div>
                    ...
                    <dd class="text-sm font-semibold text-white">
                      <?php echo e($shipment->trackingEvents->last()?->location_name ?? $shipment->sender_city); ?></dd>
                  </div>
                </div>
                <div class="flex items-start gap-3">
                  <span class="text-base mt-0.5">🏁</span>
                  <div>
                    ...
                    <dd class="text-sm font-semibold text-white"><?php echo e($shipment->receiver_address); ?></dd>
                  </div>
                </div>
              </dl>
            </aside>
          ...
          <div class="rounded-2xl border border-slate-800 bg-slate-900/80 p-6">
            ...
            <ol class="relative space-y-6 border-l-2 border-slate-800 pl-6 ml-2">
              <?php $__currentLoopData = $shipment->trackingEvents->sortByDesc('occurred_at'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $event): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php
                ...
                <li class="relative">
                  <span class="absolute -left-[1.875rem] top-1 w-4 h-4 rounded-full border-2 border-slate-950"
                    style="background:<?php echo e($dot); ?>;"></span>
                  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 mb-1">
                    <p class="font-semibold text-sm text-white"><?php echo e(str($event->status)->headline()); ?></p>
                    <time class="font-mono text-xs text-slate-500"><?php echo e($event->occurred_at?->format('M d, Y · H:i')); ?></time>
                  </div>
                  <p class="text-xs text-slate-500"><span
                      class="text-slate-400 font-medium"><?php echo e($event->location_name); ?></span>
                    <?php if($event->description): ?> · <?php echo e($event->description); ?> <?php endif; ?></p>
                </li>
...
<?php if($shipment): ?>
  <?php $__env->startPush('scripts'); ?>
    <script>
      const userRoute = <?php echo json_encode($route, 15, 512) ?>;
  ...
  <?php $__env->stopPush(); ?>
<?php endif; ?>
<?php echo $__env->make('layouts.public', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Swift-Ship\resources\views/tracking/public-show.blade.php ENDPATH**/ ?>