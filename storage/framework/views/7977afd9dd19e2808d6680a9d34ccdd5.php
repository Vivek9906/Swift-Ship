    ...
    <aside class="ops-panel rounded-lg p-4">
        ...
        <div class="mt-5 max-h-[calc(100vh-260px)] space-y-3 overflow-y-auto pr-1">
            <template x-for="shipment in shipments" :key="shipment.id">
                <button class="w-full rounded border border-slate-800 p-3 text-left transition hover:bg-slate-800/60" @click="focus(shipment)">
                    <span class="font-mono text-xs text-blue-200" x-text="shipment.tracking_number"></span>
                    <span class="mt-1 block text-sm font-semibold" x-text="shipment.customer"></span>
                    <span class="mt-1 block text-xs text-slate-400" x-text="`${shipment.origin} → ${shipment.destination} · ${shipment.eta}`"></span>
                </button>
            </template>
        </div>
    </aside>
        ...
        return {
            ...
            syncMarkers() {
                ...
                this.shipments.forEach((shipment, index) => {
                    ...
                    marker.bindPopup(`<strong>${shipment.tracking_number}</strong><br>${shipment.customer}<br>${shipment.status_label}<br>ETA ${shipment.eta}`);
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\bhard\Documents\Codex\2026-05-13\build-me-a-full-logistics-delivery\resources\views/tracking/live-map.blade.php ENDPATH**/ ?>